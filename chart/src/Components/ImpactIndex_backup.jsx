    ...
    useEffect(() => {
      const fetchData = async () => {
        ...
        if (data) {
            ...
            setContributionScoreMid(data?.contribution_mid || 0)
            setContributionScoreLow(data?.contribution_lower || 0)
            setContributionScoreHigh(data?.contribution_upper_bound || 0)
            
            setWidthScore(data?.width_score || 0);
            setContributionScore(data?.contribution_score || 0);
            setImpactIndex(data?.impact_index || 0);

            const c_low = contributionScoreLow;
            const c_mid = contributionScoreMid;
            const c_high = contributionScoreHigh;

            if ( contributionScore < c_low && contributionScore < c_mid && contributionScore < c_high ) {
                setConColor('red');
            } else if( contributionScore > c_low && contributionScore < c_mid && contributionScore < c_high ) {
                setConColor('orange');
            } else if( contributionScore > c_low && contributionScore > c_mid && contributionScore < c_high ) {
                setConColor('green');
            }
